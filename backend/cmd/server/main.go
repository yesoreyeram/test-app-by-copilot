...
func mathHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "divide":
		...
		if err != nil {
			...
			if encErr := json.NewEncoder(w).Encode(map[string]interface{}{
				...
				log.Printf("Error encoding math error response: %v", encErr)
			}
			return
		}
	default:
	...
	if err := json.NewEncoder(w).Encode(map[string]interface{}{
		...
		log.Printf("Error encoding math response: %v", err)
	}
}
...
func tempHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewEncoder(w).Encode(map[string]interface{}{
		...
		log.Printf("Error encoding temp response: %v", err)
	}
}
...
func registerHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if encErr := json.NewEncoder(w).Encode(map[string]interface{}{
			...
			log.Printf("Error encoding register error response: %v", encErr)
		}
		return
	}
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if encErr := json.NewEncoder(w).Encode(map[string]interface{}{
			...
			log.Printf("Error encoding login error response: %v", encErr)
		}
		return
	}
...
func logoutHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 2 {
		if err := userStore.DeleteSession(parts[1]); err != nil {
